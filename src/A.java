...
public class A {    //Ap=A+ADR+HVADR2=2diag+2diag2+all    A=diag
	int count;
		...
		while(!untried.isEmpty()){
			if(parent == null){
				...
				if(size<p-m){
					...
					if(!child.Dequals(child)){                                   //not contain D, only A
				...
				for(int i=0; i<parent.ycoords.length; i++){
					if(parent.ycoords[i] == parent.xcoords[i])
						size++;
					else
						size = size+2;
				}
