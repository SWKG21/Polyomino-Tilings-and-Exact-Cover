...
public class HVRI {     //from HIp, HIp=HI+HVRI+HVADR2I
	...
	public void Count(Polyomino parent, Stack<Point> untried, int p){
		...
		while(!untried.isEmpty()){
			if(parent == null){
				...
				Point imagepoint = new Point(point.x, -point.y);               //imagepoint.x=point.x, imagepoint.y=-point.y
				...
				if(size<p-m){
					...
					if(child.Vequals(child) && !child.Aequals(child)){                         //contain V, not A
