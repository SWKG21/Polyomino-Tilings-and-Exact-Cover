		...
		while(!untried.isEmpty()){
			if(parent == null){
				...
				if(size<p-1){
					...
					if(!child.Aequals(child) && !child.Hequals(child) && !child.R2equals(child) 
							&& child.Requals(child)){                                 //only R, not R2 nor H nor A
						if(this.polyominoList.isEmpty()){                             //keep one for each symmetry
							...
							while(it.hasNext()){
								...
								if(child.Requals(tmp) || child.R2equals(tmp) || child.Vequals(tmp) || child.R2equals(tmp.reflect("V"))){
									it.remove();
									this.count--;
								}
						    }
